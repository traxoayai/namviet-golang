...
const CreateB2BOrderPage = () => {
        ...
        if (isEditMode && id) {
          await salesService.updateOrder({
            ...
            p_items: verifiedItems.map((i) => ({
              ...
              discount: i.is_gift ? (i.price_wholesale || 0) * i.quantity : (i.discount || 0),
              is_gift: !!i.is_gift,
              note: "",
            })),
          });
  ...
  const handlePrintPreview = () => {
    ...
    const mockOrder = {
      ...
      note: note,
      items: items.map((i) => ({
        product_name: i.is_gift ? `[🎁 Voucher] ${i.name}` : i.name,
        uom: i.wholesale_unit,
        quantity: i.quantity,
        unit_price: i.price_wholesale,
        total_line: i.is_gift ? 0 : (i.quantity * i.price_wholesale - (i.discount || 0)),
