### Table: _revert_double_deduct_20260417 <== Backup snapshot trước khi revert 10 đơn double-deduct (2026-04-17). Giữ để rollback. An toàn xóa sau 30 ngày nếu không cần rollback.>
...
### Table: _revert_double_deduct_20260418 <== Backup đợt 2 (2026-04-18): 5 đơn double-deduct do bug case-sensitivity 20260417115000. Giữ để rollback.>
- **id**: bigint (Nullable: NO) DEFAULT nextval('_revert_double_deduct_20260418_id_seq'::regclass)
- **snapshot_at**: timestamp with time zone (Nullable: NO) DEFAULT now()
- **action**: text (Nullable: NO)
- **payload**: jsonb (Nullable: NO)

### Table: _revert_double_deduct_20260423 <== Backup snapshot trước khi revert 6 đơn double-deduct batch 3 (2026-04-23). Giữ để rollback. An toàn xóa sau 30 ngày nếu không cần.>
...
- **book_type**: character varying(20) (Nullable: YES) DEFAULT 'BOTH'::character varying

### Table: accounting_periods <== Quản lý Kỳ Kế Toán (Tháng/Năm) và trạng thái Khóa Sổ>
- **id**: bigint (Nullable: NO) DEFAULT nextval('accounting_periods_id_seq'::regclass) <== Khóa chính tự động tăng>
- **book**: text (Nullable: NO) <== Loại sổ áp dụng: INTERNAL (Sổ Nội Bộ) hoặc TAX (Sổ Thuế)>
- **year**: integer (Nullable: NO) <== Năm tài chính>
- **month**: integer (Nullable: NO) <== Tháng kế toán (1-12)>
- **status**: text (Nullable: NO) DEFAULT 'open'::text <== Trạng thái kỳ: open (Đang mở - cho phép hạch toán), closed (Đã khóa sổ - không cho phép thay đổi dữ liệu)>
- **opened_at**: timestamp with time zone (Nullable: NO) DEFAULT now() <== Thời gian bắt đầu mở kỳ (Thường là khi phát sinh giao dịch đầu tiên)>
- **closed_at**: timestamp with time zone (Nullable: YES) <== Thời gian Khóa sổ (Khi Kế toán trưởng thực hiện Chốt kỳ)>
...
- **service_type**: appointment_service_type (Nullable: YES) DEFAULT 'examination'::appointment_service_type
- **appointment_time**: timestamp with time zone (Nullable: NO)
- **status**: appointment_status (Nullable: YES) DEFAULT 'pending'::appointment_status
...
- **service_ids**: bigint[] (Nullable: YES) DEFAULT '{}'::bigint[]
...
- **exec_type**: maintenance_exec_type (Nullable: NO)
...
- **status**: asset_status (Nullable: NO) DEFAULT 'storage'::asset_status
...
### Table: b2b_notifications <== Thông báo cho khách hàng B2B trên Portal>
- **id**: uuid (Nullable: NO) DEFAULT gen_random_uuid()
- **customer_b2b_id**: bigint (Nullable: YES) <== NULL = broadcast cho tất cả khách hàng>
- **type**: b2b_notification_type (Nullable: NO) DEFAULT 'system'::b2b_notification_type
- **title**: text (Nullable: NO)
- **body**: text (Nullable: YES)
- **data**: jsonb (Nullable: YES) DEFAULT '{}'::jsonb <== JSON context: order_id, invoice_id, url, ...>
- **is_read**: boolean (Nullable: NO) DEFAULT false
- **read_at**: timestamp with time zone (Nullable: YES)
- **created_at**: timestamp with time zone (Nullable: NO) DEFAULT now()

### Table: b2b_push_subscriptions <== Web Push subscriptions cho portal users>
- **id**: uuid (Nullable: NO) DEFAULT gen_random_uuid()
...
### Table: batch_revaluations <== Audit trail cho mọi lần điều chỉnh giá vốn theo lô (batches.inbound_price)>
...
- **delta_value**: numeric (Nullable: YES) DEFAULT ((qty_at_change)::numeric * (new_price - old_price))
...
### Table: bctc_line_mapping
- **id**: bigint (Nullable: NO) DEFAULT nextval('bctc_line_mapping_id_seq'::regclass)
- **report**: text (Nullable: NO)
- **ma_so**: text (Nullable: NO)
- **ten_chi_tieu**: text (Nullable: NO)
- **sort_order**: integer (Nullable: NO)
- **account_prefix**: text (Nullable: NO)
- **sign**: integer (Nullable: NO) DEFAULT 1
- **side**: text (Nullable: NO) DEFAULT 'net'::text
...
- **type**: account_type (Nullable: NO)
- **balance_type**: account_balance_type (Nullable: NO)
- **status**: account_status (Nullable: NO) DEFAULT 'active'::account_status
- **allow_posting**: boolean (Nullable: NO) DEFAULT true
- **created_at**: timestamp with time zone (Nullable: YES) DEFAULT now()
- **updated_at**: timestamp with time zone (Nullable: YES) DEFAULT now()

### Table: chat_cache <== Response cache 10 phút cho chatbot — key = sha256(user_id|normalized_query). RLS chỉ service_role.>
...
- **matched_keywords**: text[] (Nullable: YES)
...
### Table: chat_feedback <== Phản hồi sales về chất lượng bot — dùng cho training data.>
- **id**: uuid (Nullable: NO) DEFAULT gen_random_uuid()
- **message_id**: uuid (Nullable: NO)
...
### Table: chat_feedback_weekly_clusters <== C7 feedback loop: cụm keyword frequent từ wrong_answer feedback theo tuần.>
...
- **sample_message_ids**: uuid[] (Nullable: NO)
- **feedback_count**: integer (Nullable: NO)
- **created_at**: timestamp with time zone (Nullable: NO) DEFAULT now()

### Table: chat_handoffs <== Yêu cầu chuyển phiên chat từ bot sang sales — resolved_at NULL = chờ xử lý.>
- **id**: uuid (Nullable: NO) DEFAULT gen_random_uuid()
- **session_id**: uuid (Nullable: NO)
...
### Table: chat_messages <== Tin nhắn trong phiên chat — soft delete qua deleted_at để audit.>
- **id**: uuid (Nullable: NO) DEFAULT gen_random_uuid()
- **session_id**: uuid (Nullable: NO)
...
### Table: chat_rate_limit <== Sliding-bucket counter cho chat rate-limit (60 msg/phút/user mặc định). Service-role only; deny-all RLS với client roles.>
- **user_id**: uuid (Nullable: NO)
- **window_start**: timestamp with time zone (Nullable: NO)
- **count**: integer (Nullable: NO) DEFAULT 0
- **updated_at**: timestamp with time zone (Nullable: NO) DEFAULT now()

### Table: chat_sessions <== Phiên chat Phase 1 MVP — 1 user có thể có nhiều phiên (mỗi platform 1 phiên active).>
- **id**: uuid (Nullable: NO) DEFAULT gen_random_uuid()
- **user_id**: uuid (Nullable: NO)
...
- **status**: queue_status (Nullable: YES) DEFAULT 'waiting'::queue_status
- **priority_level**: queue_priority (Nullable: YES) DEFAULT 'normal'::queue_priority
...
### Table: connect_posts <== Lưu trữ Thông báo, Góp ý và Tài liệu nội bộ>
- **id**: bigint (Nullable: NO)
- **created_at**: timestamp with time zone (Nullable: YES) DEFAULT now()
...
- **tags**: text[] (Nullable: YES) DEFAULT '{}'::text[]
...
### Table: customer_segments <== Lưu trữ các nhóm khách hàng (Tĩnh và Động)>
- **id**: bigint (Nullable: NO)
- **name**: text (Nullable: NO)
- **description**: text (Nullable: YES)
...
- **name**: text (Nullable: NO)
- **type**: customer_b2c_type (Nullable: NO) DEFAULT 'CaNhan'::customer_b2c_type
- **phone**: text (Nullable: YES)
- **email**: text (Nullable: YES)
- **address**: text (Nullable: YES)
- **dob**: date (Nullable: YES)
- **gender**: customer_gender (Nullable: YES)
...
- **updated_by**: uuid (Nullable: YES)
- **customer_type**: character varying(10) (Nullable: YES) DEFAULT 'B2C'::character varying
...
- **customer_type**: character varying(10) (Nullable: YES) DEFAULT 'B2B'::character varying
...
- **override_discount_value**: numeric(12,2) (Nullable: YES)
- **created_at**: timestamp with time zone (Nullable: NO) DEFAULT now()
...
- **district_codes**: text[] (Nullable: NO)
- **is_active**: boolean (Nullable: YES) DEFAULT true
- **created_at**: timestamp with time zone (Nullable: YES) DEFAULT now()
...
- **district_codes**: text[] (Nullable: YES)
- **reason**: text (Nullable: YES)
- **created_at**: timestamp with time zone (Nullable: YES) DEFAULT now()
...
- **name**: text (Nullable: NO)
- **module**: template_module (Nullable: NO)
- **type**: template_type (Nullable: NO)
- **status**: account_status (Nullable: NO) DEFAULT 'active'::account_status
- **content**: text (Nullable: YES)
- **created_at**: timestamp with time zone (Nullable: YES) DEFAULT now()
- **updated_at**: timestamp with time zone (Nullable: YES) DEFAULT now()
...
- **items_json**: jsonb (Nullable: YES) DEFAULT '[]'::jsonb <== Chứa mảng items: [{name, unit, qty, price, total, vat_rate, lot, exp}]>
...
- **total_fee_amount**: numeric (Nullable: YES) DEFAULT 0 <== Tổng tiền phí mua hàng (nhập tay) — phân bổ vào giá vốn tồn kho theo tỷ trọng Thành tiền từng dòng (chuẩn VAS). Có thể không chịu VAT.>
- **total_goods_amount**: numeric (Nullable: NO) DEFAULT 0 <== Tổng tiền hàng nguyên giá = SUM(SL*ĐG) trước chiết khấu.>
- **total_discount_amount**: numeric (Nullable: NO) DEFAULT 0 <== Tổng chiết khấu thương mại (XML: TTCKTMai = SUM STCKhau từng dòng).>
...
- **sepay_tracking_code**: text (Nullable: YES)
- **sepay_reference_code**: text (Nullable: YES)
- **sepay_status**: text (Nullable: YES)

### Table: finance_transactions
- **id**: bigint (Nullable: NO)
- **code**: text (Nullable: NO)
- **transaction_date**: timestamp with time zone (Nullable: NO) DEFAULT now()
- **flow**: transaction_flow (Nullable: NO)
- **business_type**: business_type (Nullable: NO) DEFAULT 'other'::business_type
...
- **created_by**: uuid (Nullable: YES) DEFAULT auth.uid()
- **created_at**: timestamp with time zone (Nullable: YES) DEFAULT now()
- **status**: transaction_status (Nullable: NO) DEFAULT 'pending'::transaction_status
...
- **book_type**: text (Nullable: NO) DEFAULT 'BOTH'::text <== Sổ ghi nhận phiếu: INTERNAL (nội bộ) | TAX (thuế) | BOTH (cả 2). Mặc định BOTH.>
- **is_posted**: boolean (Nullable: NO) DEFAULT false <== Đánh dấu Phiếu Thu/Chi đã được Hạch toán vào Sổ cái hay chưa>

### Table: fund_accounts
- **id**: bigint (Nullable: NO) DEFAULT nextval('fund_accounts_id_seq'::regclass)
- **name**: text (Nullable: NO)
- **type**: fund_account_type (Nullable: NO)
...
- **status**: fund_account_status (Nullable: NO) DEFAULT 'active'::fund_account_status
- **created_at**: timestamp with time zone (Nullable: YES) DEFAULT now()
- **updated_at**: timestamp with time zone (Nullable: YES) DEFAULT now()
...
- **updated_at**: timestamp with time zone (Nullable: YES) DEFAULT now()
- **created_by**: uuid (Nullable: YES)
- **diff_quantity**: numeric (Nullable: YES) DEFAULT (actual_quantity - system_quantity)
...
- **total_diff_value**: numeric (Nullable: YES) DEFAULT (total_actual_value - total_system_value)
- **status**: text (Nullable: YES) DEFAULT 'DRAFT'::text
- **note**: text (Nullable: YES)
- **created_by**: uuid (Nullable: YES)
...
- **discount_amount**: numeric (Nullable: YES) DEFAULT 0
- **vat_rate**: numeric (Nullable: YES) DEFAULT 0
- **sub_total**: numeric (Nullable: YES) DEFAULT (((quantity)::numeric * unit_price) - discount_amount)
...
- **total_value**: numeric (Nullable: YES) DEFAULT (quantity * unit_price)

### Table: inventory_transfer_batch_items <== Chi tiết Lô hàng được chỉ định để xuất kho (FEFO)>
- **id**: bigint (Nullable: NO)
...
### Table: inventory_transfer_items <== Chi tiết sản phẩm cần chuyển (Chưa định danh Lô)>
- **id**: bigint (Nullable: NO)
...
### Table: inventory_transfers <== Phiếu chuyển kho (Header)>
- **id**: bigint (Nullable: NO)
- **code**: text (Nullable: NO)
...
### Table: llm_request_log <== Log mỗi attempt gọi LLM provider (success/rate_limit/error/tool_use_failed). RLS chỉ chat_staff đọc.>
...
- **status**: text (Nullable: NO) <== Trạng thái attempt LLM. Enum hợp lệ: 'success' | 'rate_limit' | 'circuit_open' | 'tool_use_failed' | 'error'. Không có CHECK constraint để tương thích legacy data.>
- **latency_ms**: integer (Nullable: YES)
- **tokens_in**: integer (Nullable: YES)
- **tokens_out**: integer (Nullable: YES)
- **error_message**: text (Nullable: YES)
- **attempted_providers**: text[] (Nullable: YES)
- **created_at**: timestamp with time zone (Nullable: NO) DEFAULT now()
...
- **temperature**: numeric(4,1) (Nullable: YES)
...
- **weight**: numeric(5,2) (Nullable: YES)
- **height**: numeric(5,2) (Nullable: YES)
- **bmi**: numeric(4,2) (Nullable: YES)
- **head_circumference**: numeric(4,1) (Nullable: YES)
- **birth_weight**: numeric(5,2) (Nullable: YES)
- **birth_height**: numeric(5,2) (Nullable: YES)
...
- **metadata**: jsonb (Nullable: YES) DEFAULT '{}'::jsonb <== Dữ liệu bổ sung cho navigation: entity id, code, ...>
...
- **base_quantity**: integer (Nullable: YES) DEFAULT (quantity * conversion_factor)
...
- **expiry_date**: date (Nullable: YES)
- **total_line**: numeric (Nullable: YES) DEFAULT (((quantity)::numeric * unit_price) - discount)
- **created_at**: timestamp with time zone (Nullable: YES) DEFAULT now()
- **quantity_picked**: integer (Nullable: YES) DEFAULT 0
- **quantity_returned**: integer (Nullable: YES) DEFAULT 0

### Table: order_status_history <== Audit log cho orders.status transitions. Trigger trg_order_status_history insert mỗi row update status. read-only cho authenticated.>
...
- **shipping_fee**: numeric (Nullable: YES) DEFAULT 0 <== Phí ship snapshot lúc đặt đơn. Đơn cũ = 0 (default).>
- **discount_amount**: numeric (Nullable: YES) DEFAULT 0
...
- **warehouse_id**: bigint (Nullable: YES)
- **invoice_status**: invoice_request_status (Nullable: YES) DEFAULT 'none'::invoice_request_status
...
### Table: portal_cart_items <== Giỏ hàng Portal — mỗi portal_user có giỏ riêng>
- **id**: uuid (Nullable: NO) DEFAULT gen_random_uuid()
- **portal_user_id**: uuid (Nullable: NO)
- **product_id**: bigint (Nullable: NO)
- **quantity**: integer (Nullable: NO)
...
- **seo_keywords**: text[] (Nullable: YES)
...
- **discount_value**: numeric(12,2) (Nullable: NO)
...
- **product_id**: integer (Nullable: NO)
- **content**: text (Nullable: NO)
- **embedding**: vector(768) (Nullable: NO)
- **updated_at**: timestamp with time zone (Nullable: NO) DEFAULT now()
...
- **updated_at**: timestamp with time zone (Nullable: YES) DEFAULT now() <== Thời gian cập nhật tồn kho hoặc cấu hình Min/Max>
...
### Table: product_synonyms <== Từ đồng nghĩa cho search SP qua chatbot — xa20, xarelto20, rivaroxaban... Master data, không user-specific.>
...
- **price**: numeric (Nullable: YES) DEFAULT 0 <== Giá bán ra áp dụng cho đơn vị này>
...
- **image_url**: text (Nullable: YES)
- **status**: text (Nullable: NO) DEFAULT 'active'::text
- **fts**: tsvector (Nullable: YES) DEFAULT to_tsvector('simple'::regconfig, ((((((COALESCE(name, ''::text) || ' '::text) || COALESCE(sku, ''::text)) || ' '::text) || COALESCE(active_ingredient, ''::text)) || ' '::text) || COALESCE(barcode, ''::text)))
- **created_at**: timestamp with time zone (Nullable: YES) DEFAULT now()
- **updated_at**: timestamp with time zone (Nullable: YES) DEFAULT now()
...
- **stock_management_type**: stock_management_type (Nullable: YES) DEFAULT 'lot_date'::stock_management_type
- **wholesale_margin_rate**: numeric (Nullable: YES) DEFAULT 0
- **retail_margin_rate**: numeric (Nullable: YES) DEFAULT 0
- **usage_instructions**: jsonb (Nullable: YES) DEFAULT '{}'::jsonb <== HDSD phân theo nhóm tuổi: 0_2, 2_6, 6_18, 18_plus>
- **updated_by**: uuid (Nullable: YES)
- **category_id**: integer (Nullable: YES)
- **manufacturer_id**: integer (Nullable: YES)
- **stock_status**: text (Nullable: YES) DEFAULT 'in_stock'::text
- **product_images**: text[] (Nullable: YES) DEFAULT '{}'::text[] <== Mảng chứa các đường dẫn hình ảnh phụ của sản phẩm>

### Table: promotion_gifts
- **id**: bigint (Nullable: NO)
- **name**: text (Nullable: NO)
- **type**: gift_type (Nullable: NO)
...
### Table: registration_requests <== Yêu cầu đăng ký tài khoản portal B2B - admin duyệt trước khi tạo account>
- **id**: uuid (Nullable: NO) DEFAULT gen_random_uuid()
...
- **customer_b2c_id**: bigint (Nullable: YES)
- **note**: text (Nullable: YES)
- **status**: text (Nullable: NO) DEFAULT 'pending'::text <== pending: Mới tạo/Chờ xử lý
...
- **type**: service_package_type (Nullable: NO) DEFAULT 'service'::service_package_type
...
- **applicable_branches**: bigint[] (Nullable: YES)
...
- **district_code**: text (Nullable: YES)
...
### Table: shipping_fee_config <== Phí ship flat rate per method. Phase 1 — không zone, không weight. Admin update qua SQL/Studio.>
- **id**: bigint (Nullable: NO)
...
- **name**: text (Nullable: NO)
- **type**: shipping_partner_type (Nullable: NO) DEFAULT 'app'::shipping_partner_type
...
- **code**: text (Nullable: YES)
- **name**: text (Nullable: NO)
- **type**: supplier_program_type (Nullable: NO)
...
### Table: tasks <== Trung tâm quản lý tác vụ và giao việc tự động (Task Hub)>
- **id**: uuid (Nullable: NO) DEFAULT gen_random_uuid()
- **title**: text (Nullable: NO)
- **description**: text (Nullable: YES)
...
- **entity_type**: text (Nullable: YES) DEFAULT 'none'::text <== Loại thực thể liên quan: order, customer, product, inventory_batch...>
- **entity_id**: text (Nullable: YES)
- **due_date**: timestamp with time zone (Nullable: NO)
- **completed_at**: timestamp with time zone (Nullable: YES)
- **kpi_points**: integer (Nullable: YES) DEFAULT 0 <== Điểm thưởng Gamification: +10 (Sớm), +5 (Trễ hạn)>
...
- **code**: text (Nullable: NO)
- **name**: text (Nullable: NO)
- **type**: transaction_type (Nullable: NO)
- **account_id**: text (Nullable: YES)
- **status**: account_status (Nullable: NO) DEFAULT 'active'::account_status
- **description**: text (Nullable: YES)
- **created_at**: timestamp with time zone (Nullable: YES) DEFAULT now()
...
- **status**: account_status (Nullable: YES) DEFAULT 'active'::account_status
- **created_at**: timestamp with time zone (Nullable: YES) DEFAULT now()
- **updated_at**: timestamp with time zone (Nullable: YES) DEFAULT now()
...
- **position**: text (Nullable: YES)
- **status**: employee_status (Nullable: NO) DEFAULT 'pending_approval'::employee_status
- **dob**: date (Nullable: YES)
- **phone**: text (Nullable: YES)
...
### Table: vat_inventory_ledger <== Sổ cái theo dõi tồn kho Hóa đơn VAT (Lưu trữ theo Base Unit và Tax Rate)>
- **id**: bigint (Nullable: NO)
- **product_id**: bigint (Nullable: NO)
