CREATE OR REPLACE FUNCTION public.create_sales_order(p_items jsonb, p_customer_id bigint DEFAULT NULL::bigint, p_customer_b2b_id bigint DEFAULT NULL::bigint, p_customer_b2c_id bigint DEFAULT NULL::bigint, p_status text DEFAULT 'CONFIRMED'::text, p_payment_method text DEFAULT 'credit'::text, p_discount_amount numeric DEFAULT 0, p_shipping_fee numeric DEFAULT 0, p_shipping_partner_id bigint DEFAULT NULL::bigint, p_delivery_method text DEFAULT NULL::text, p_delivery_address text DEFAULT NULL::text, p_delivery_time timestamp with time zone DEFAULT NULL::timestamp with time zone, p_note text DEFAULT NULL::text, p_warehouse_id bigint DEFAULT NULL::bigint, p_order_type text DEFAULT NULL::text, p_voucher_code text DEFAULT NULL::text, p_source text DEFAULT 'erp'::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
