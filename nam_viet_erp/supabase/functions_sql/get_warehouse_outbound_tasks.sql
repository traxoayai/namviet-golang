CREATE OR REPLACE FUNCTION public.get_warehouse_outbound_tasks(p_page integer DEFAULT 1, p_page_size integer DEFAULT 20, p_search text DEFAULT NULL::text, p_status text DEFAULT NULL::text, p_type text DEFAULT NULL::text, p_date_from date DEFAULT NULL::date, p_date_to date DEFAULT NULL::date, p_warehouse_id bigint DEFAULT 1, p_shipping_partner_id bigint DEFAULT NULL::bigint)
 RETURNS TABLE(task_id uuid, code text, task_type text, customer_name text, created_at timestamp with time zone, delivery_deadline timestamp with time zone, priority text, status text, shipping_partner_name text, shipping_contact_name text, shipping_contact_phone text, package_count integer, progress_picked integer, progress_total integer, status_label text, total_count bigint)
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
DECLARE
