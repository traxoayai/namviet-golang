CREATE OR REPLACE FUNCTION public.search_products_for_purchase(p_keyword text DEFAULT ''::text)
 RETURNS TABLE(id bigint, name text, sku text, barcode text, image_url text, wholesale_unit text, retail_unit text, items_per_carton integer, actual_cost numeric, latest_purchase_price numeric, total_stock integer, avg_monthly_sold numeric, formatted_monthly_sales_qty text, available_units jsonb)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
    ...
    v_sql := format($q$
        SELECT
            ...
            -- [NEW] Calculate formatted_monthly_sales_qty using the helper function
            public.format_product_quantity(
                COALESCE(
                    (
                        SELECT ROUND(SUM(oi.quantity * COALESCE(oi.conversion_factor, 1)) / 3.0, 1)
                        FROM public.order_items oi
                ...
                p.id
            ) AS formatted_monthly_sales_qty,
