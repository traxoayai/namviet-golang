-- Migration: create_sales_order — ngăn orders.status = NULL lọt khỏi filter
-- ============================================================================
-- BUG (data-integrity-p0 plan, Task 4):
--   Cột orders.status cho phép NULL (is_nullable = YES) và nếu caller gọi
--   create_sales_order() với p_status = NULL (explicit), thì INSERT ghi thẳng
--   NULL vào orders.status (bypass column DEFAULT 'PENDING'). Đơn NULL lọt
--   khỏi mọi filter (warehouse outbound, sales view, trigger deduct, cron).
--
-- STATE THỰC TẾ TRÊN PROD (verified 2026-04-23):
--   - column orders.status: column_default='PENDING', is_nullable=YES
--   - RPC signature: 17 params, RETURNS jsonb, p_status DEFAULT 'CONFIRMED'
--     (bản `20260417190000_restore_disable_credit_check_keep_b2b_warehouse`)
--   - COUNT(*) WHERE status IS NULL = 0 → an toàn SET NOT NULL.
--
-- FIX — 2 layer:
--   (a) DB-level: SET NOT NULL (DEFAULT 'PENDING' đã có sẵn, giữ nguyên).
--   (b) RPC-level: COALESCE(p_status, 'PENDING') tại INSERT —
--       belt-and-suspenders phòng caller truyền explicit NULL.
--       GIỮ NGUYÊN default 'CONFIRMED' cho p_status để không đổi behavior
--       hiện có (đa số caller kỳ vọng tạo đơn CONFIRMED).
--
-- Source body: copy verbatim từ
-- 20260417190000_restore_disable_credit_check_keep_b2b_warehouse.sql,
-- CHỈ đổi 1 dòng INSERT (p_status → COALESCE(p_status, 'PENDING')).
--
-- Date: 2026-04-23
-- ============================================================================
...
-- (b) RPC-level: COALESCE belt-and-suspenders
...
BEGIN
  ...
  -- [FIX — Bug #2] Normalize status: nếu caller truyền NULL thì fallback 'PENDING'.
  -- Belt-and-suspenders cho DB-level NOT NULL vừa thêm.
  ...
  -- [FIX] status = v_safe_status (đã COALESCE) thay vì p_status trực tiếp
