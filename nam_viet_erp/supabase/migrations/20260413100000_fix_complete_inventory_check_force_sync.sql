-- Fix complete_inventory_check: force-sync product_inventory sau khi xử lý
-- Bug: Sản phẩm có stock_quantity trong product_inventory nhưng KHÔNG có record
-- trong inventory_batches (data seed/import mặc định). Khi kiểm kê:
--   1. batch_code = NULL → refresh set system_quantity = 0
--   2. diff = 0 - 0 = 0 → không trừ kho
--   3. product_inventory.stock_quantity vẫn giữ nguyên (VD: 999)
-- Fix: Thêm bước force-sync product_inventory = SUM(inventory_batches) cho
-- TẤT CẢ sản phẩm trong phiếu kiểm kê sau khi xử lý xong.
-- 2026-04-13
...
BEGIN
    ...
    -- [FIX v2] Refresh system_quantity = tồn kho HIỆN TẠI từ inventory_batches
    ...
    -- Giải quyết: sản phẩm có stock_quantity trong product_inventory nhưng không có
    -- inventory_batches (data import/seed), hoặc trigger sync bị miss edge-case.
