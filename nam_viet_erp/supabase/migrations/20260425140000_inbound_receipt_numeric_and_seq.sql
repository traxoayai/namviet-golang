-- Migration: process_inbound_receipt — NUMERIC qty/rate + sequential PNK code
-- ============================================================================
-- BUG 1: v_qty_base INTEGER, v_conversion_rate INTEGER → truncation khi nhân
--         qty thập phân (vd: 1.5 chai × 400 viên = 600 nhưng INTEGER truncates).
-- BUG 2: PNK code dùng random()*10000 → collision khi nhiều phiếu cùng lúc.
-- FIX 1: Đổi sang NUMERIC để giữ precision.
-- FIX 2: Dùng public._gen_finance_tx_code('PNK') → nextval sequence, unique.
-- Giữ nguyên toàn bộ logic: fail-fast unit check, batch upsert, PO status.
-- Date: 2026-04-25
-- ============================================================================
