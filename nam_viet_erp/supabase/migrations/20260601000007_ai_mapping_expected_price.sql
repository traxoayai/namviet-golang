-- Migration: ai_mapping_expected_price
-- Description: Update mapped_invoice_product and map_scanned_invoice_products to return expected_pre_vat_price and expected_vat from vendor_product_mappings.
...
-- 2. Alter the type to add expected price and vat
DO $$ 
BEGIN
    ALTER TYPE public.mapped_invoice_product ADD ATTRIBUTE expected_pre_vat_price numeric CASCADE;
EXCEPTION
...
DO $$ 
BEGIN
    ALTER TYPE public.mapped_invoice_product ADD ATTRIBUTE expected_vat numeric CASCADE;
EXCEPTION
    WHEN duplicate_column THEN NULL;
END $$;
    ...
    LOOP
        ...
        IF v_vendor_tax_code IS NOT NULL AND v_vendor_tax_code != '' THEN
            ...
            IF v_sku IS NOT NULL AND v_sku != '' THEN
                SELECT m.internal_product_id, p.name, m.internal_product_unit_id, m.pre_vat_price, m.vat_of_supplier
                INTO v_internal_id, v_internal_name, v_internal_product_unit_id, v_expected_price, v_expected_vat
        ...
        RETURN QUERY SELECT 
            ...
            v_internal_id, v_internal_name, v_score, v_method, v_internal_product_unit_id, v_expected_price, v_expected_vat;
    END LOOP;
END;
$$;
