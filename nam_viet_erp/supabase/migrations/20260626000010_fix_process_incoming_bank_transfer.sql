-- HOTFIX: process_incoming_bank_transfer
-- Vấn đề 1: ref_id đang lưu order.code thay vì order.id (UUID) → Backend không thể lookup
-- Vấn đề 2: Sau khi gạch nợ không update payment_status của đơn hàng
-- Giải pháp: Lưu ref_id = order.id (UUID), thêm UPDATE payment_status sau mỗi INSERT
-- ============================================================================
...
DECLARE
  ...
  v_new_paid numeric;
BEGIN
  -- Idempotency: bỏ qua nếu đã xử lý bank_reference_id này rồi
  ...
  -- Không tìm thấy mã đơn trong memo → lưu phiếu "chưa rõ"
  ...
  IF array_length(v_codes, 1) = 1 OR v_total_outstanding = 0 THEN
    ...
    SELECT o.id, o.code, o.customer_id, o.customer_b2c_id,
           o.final_amount, COALESCE(o.paid_amount, 0) AS paid_amount,
    ...
    IF v_single_alloc > 0 THEN
      ...
      INSERT INTO public.finance_transactions (
        ...
        'ORDER', v_order.id::TEXT,  -- [FIX] Lưu UUID thay vì code
        'Hệ thống tự động gạch nợ. ND gốc: ' || COALESCE(p_memo, ''),
        'completed', p_bank_ref_id
      );

      -- [FIX] Cập nhật paid_amount và payment_status đơn hàng
      v_new_paid := v_order.paid_amount + v_single_alloc;
      UPDATE public.orders
      SET
        payment_method = 'bank_transfer',
        paid_amount    = v_new_paid,
        payment_status = CASE
          WHEN v_new_paid >= v_order.final_amount THEN 'paid'
          ELSE 'partial'
        END
      WHERE id = v_order.id;
  ...
  LOOP
    ...
    -- [FIX] Cập nhật paid_amount và payment_status đơn hàng
    v_new_paid := v_order.paid_amount + v_alloc_amount;
    UPDATE public.orders
    SET
      payment_method = 'bank_transfer',
      paid_amount    = v_new_paid,
