-- HOTFIX B7 + C5: clone_sales_order preserve đủ header fields + composite index
-- ============================================================================
-- B7: Clone hiện chỉ copy 17 cột hard-coded (delivery_address, shipping_fee,
--     payment_method, ...). Đơn thực tế có thêm: source, delivery_province_id,
--     delivery_district_id, delivery_ward_id, fee_payer, package_count,
--     invoice_status, invoice_request_data, remittance_transaction_id,
--     quote_expires_at. Clone mất → đơn sao mất context (source='portal'
--     → NULL → stat lệch; address chi tiết mất → không in được ship nhãn).
--
-- FIX: Copy tất cả field đơn gốc, OVERRIDE đúng những cái reset:
--   id = gen_random_uuid() (auto)
--   code = new v_new_code
--   status = 'DRAFT', payment_status = 'unpaid', remittance_status = derived
--   paid_amount = 0, total_amount = 0, final_amount = 0 (recalc sau)
--   note = append "(Nhân bản từ ...)"
--   creator_id = auth.uid()
--   created_at = updated_at = NOW()
--   remittance_transaction_id = NULL
-- Các field khác clone y nguyên đơn gốc.
--
-- C5: Composite index (order_id, created_at DESC) cho query audit của 1 đơn
--     — thay thế 2 single-column indexes hiện tại.
-- Date: 2026-04-23
-- ============================================================================
...
-- Giữ single index trên created_at cho query "recent transitions system-wide"
-- (dashboard). Thêm composite cho query per-order.
CREATE INDEX IF NOT EXISTS order_status_history_order_id_created_at_idx
  ON public.order_status_history(order_id, created_at DESC);

-- B7: Re-create clone_sales_order — dùng INSERT INTO ... SELECT để auto-copy
-- tất cả field, OVERRIDE một số.
...
BEGIN
    ...
    ELSE
        v_prefix := 'SO-';
    END IF;
    v_new_code := v_prefix || to_char(now(), 'YYMMDD') || '-' ||
