-- Modify create_customer_b2b and update_customer_b2b to include sales_permissions
...
BEGIN
  ...
  VALUES (
    ...
    (SELECT array_agg(x) FROM jsonb_array_elements_text(p_customer_data->'sales_permissions') x)
  )
...
BEGIN
  ...
  SET
    ...
    business_license_url = p_customer_data->>'business_license_url',
    ...
    loyalty_points = (p_customer_data->>'loyalty_points')::INT,
    sales_permissions = (SELECT array_agg(x) FROM jsonb_array_elements_text(p_customer_data->'sales_permissions') x),
    updated_at = now()
  WHERE id = p_id;

  -- 2. Cập nhật Liên hệ (Xóa cũ, Thêm mới)
  DELETE FROM public.customer_b2b_contacts WHERE customer_id = p_id;
