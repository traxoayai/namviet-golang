CREATE OR REPLACE FUNCTION public.update_sales_order(p_order_id uuid, p_customer_id bigint, p_delivery_address text, p_delivery_time text, p_note text, p_discount_amount numeric, p_shipping_fee numeric, p_items jsonb, p_status text DEFAULT 'DRAFT'::text)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
    ...
    -- 2. Update Header (Thông tin chung - KHÔNG UPDATE STATUS Ở ĐÂY ĐỂ TRÁNH TRIGGER CHẠY SỚM)
    ...
    -- Update lại số tiền vào Header VÀ TRẠNG THÁI (để trigger trừ kho chạy đúng với items mới nhất)
    UPDATE public.orders
