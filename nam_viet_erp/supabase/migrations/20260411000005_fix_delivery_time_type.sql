-- Fix: create_sales_order lỗi khi gửi p_delivery_time dạng text
-- Nguyên nhân: Migration trước đổi p_delivery_time từ TEXT → TIMESTAMPTZ
-- nhưng cột orders.delivery_time vẫn là TEXT, frontend gửi text.
-- Fix: Đổi lại về TEXT cho khớp với cột DB và frontend.

-- Cần DROP bản TIMESTAMPTZ trước rồi CREATE lại với TEXT
-- vì đổi param type = function khác (PostgreSQL overload rules)

-- Drop bản 16-param với TIMESTAMPTZ (nếu có)
DROP FUNCTION IF EXISTS public.create_sales_order(
  JSONB, BIGINT, BIGINT, BIGINT, TEXT, TEXT, NUMERIC, NUMERIC,
  BIGINT, TEXT, TEXT, TIMESTAMPTZ, TEXT, BIGINT, TEXT, TEXT
);
-- Drop bản 15-param với TIMESTAMPTZ (không có p_voucher_code)
DROP FUNCTION IF EXISTS public.create_sales_order(
  JSONB, BIGINT, BIGINT, BIGINT, TEXT, TEXT, NUMERIC, NUMERIC,
  BIGINT, TEXT, TEXT, TIMESTAMPTZ, TEXT, BIGINT, TEXT
);
...
BEGIN
  ...
  -- E. CREDIT EXPOSURE CHECK (B2B) - Tạm bỏ
  ...
  LOOP
    ...
    -- Không trừ kho cho B2B khi status = CONFIRMED
