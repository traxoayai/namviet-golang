-- Migration: 20260121_upgrade_sales_order_view_v9_warehouse.sql
-- Description: Thêm thông tin Warehouse Name vào RPC danh sách đơn hàng
    ...
    BEGIN
        ...
        WITH filtered_data AS (
            SELECT 
                ...
                -- Invoice Data (Giữ nguyên cho nút VAT)
                ...
                -- Items Data (Giữ nguyên cho nút VAT)
                (
            ...
            LEFT JOIN public.warehouses w ON o.warehouse_id = w.id -- [JOIN WAREHOUSE]
            WHERE 
                (p_order_type IS NULL OR o.order_type = p_order_type)
                ...
                -- SEARCH (Giữ nguyên V8.1 - đã fix lỗi ambiguous)
                AND (
