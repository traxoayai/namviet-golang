-- HOTFIX: process_incoming_bank_transfer idempotent check miss suffix rows
-- ============================================================================
-- BUG: 240000 fix B3/B4/C1 thêm suffix vào bank_reference_id cho tx dư
--      (-excess / -remainder / -<order_code>) để tránh UNIQUE VIOLATION nội
--      bộ. NHƯNG idempotent check đầu hàm vẫn là exact-match:
--        EXISTS (SELECT 1 FROM finance_transactions WHERE bank_reference_id = p_bank_ref_id)
--      → Webhook ngân hàng retry lần 2 với CÙNG p_bank_ref_id:
--        - Lần 1 DB đã có row với bank_reference_id = '<ref>-<code>' (không match exact)
--        - Check exist → false → hàm tiếp tục chạy INSERT
--        - Lúc này UNIQUE(bank_reference_id) sẽ throw 23505 → rollback all
--          → trigger webhook ngân hàng cho là fail, có thể retry vô hạn
--
-- FIX: Check bank_reference_id LIKE p_bank_ref_id || '%' — cover mọi suffix
--      do hàm này tự sinh. Giữ nguyên toàn bộ logic còn lại.
-- Date: 2026-04-23
-- ============================================================================
...
DECLARE
  ...
  v_excess numeric;
BEGIN
  -- [FIX] Check prefix match: cover cả exact ref + ref + '-<suffix>'
