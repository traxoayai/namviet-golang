-- HOTFIX: migration 20260424000000 tạo nhầm overload 16-param cho
-- create_sales_order (sai signature), trùng với bản canonical 17-param từ
-- 20260423200200 → PostgREST ambiguous "Could not choose the best candidate".
--
-- FIX:
-- 1. Drop 16-param overload sai.
-- 2. CREATE OR REPLACE 17-param canonical, GIỮ NGUYÊN toàn bộ logic +
--    chỉ thêm 1 RAISE EXCEPTION khi v_final_b2b_id IS NULL cho đơn B2B
--    (đây mới là mục tiêu #9 gốc).
-- Date: 2026-04-24
-- ============================================================================

BEGIN;

-- 1. Drop overload bogus
DROP FUNCTION IF EXISTS public.create_sales_order(
  jsonb, bigint, bigint, bigint, text, bigint, text, text, text, text,
  text, text, text, text, date, text
);

-- 2. Replace canonical 17-param — copy full 20260423200200 + thêm B2B NULL check
...
BEGIN
  ...
  -- [FIX #9] Đơn B2B bắt buộc có customer_b2b_id. Trước đây silent NULL →
  -- đơn lọt với customer_id NULL → công nợ/dashboard mất trace.
