-- Portal B2B: danh sách hóa đơn bán (sales_invoices) theo khách hàng.
-- Khác finance_invoices (hóa đơn mua / VAT nhập kho trong ERP).
...
COMMENT ON FUNCTION public.get_customer_sales_invoices IS
  'Portal B2B: list sales_invoices for customers_b2b with optional status/search/pagination.';

GRANT EXECUTE ON FUNCTION public.get_customer_sales_invoices(BIGINT, TEXT, TEXT, INT, INT) TO anon;
GRANT EXECUTE ON FUNCTION public.get_customer_sales_invoices(BIGINT, TEXT, TEXT, INT, INT) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_customer_sales_invoices(BIGINT, TEXT, TEXT, INT, INT) TO service_role;
