-- Add quantity_buyer column
ALTER TABLE finance_invoice_items ADD COLUMN IF NOT EXISTS quantity_buyer NUMERIC;

-- Recreate the upsert_finance_invoice RPC
CREATE OR REPLACE FUNCTION public.upsert_finance_invoice(p_invoice_data jsonb, p_items_data jsonb)
 RETURNS bigint
 LANGUAGE plpgsql
AS $function$
DECLARE
...
BEGIN
    ...
    v_direction := COALESCE(p_invoice_data->>'direction', 'inbound');

    IF v_invoice_id IS NULL THEN
        -- Check existing invoice based on direction
        ...
        INSERT INTO public.finance_invoices (
            ...
            supplier_name_raw, supplier_tax_code, supplier_id, supplier_address_raw,
            buyer_name, buyer_company_name, buyer_address, buyer_email, buyer_tax_code,
            customer_id, customer_b2c_id, payment_method, order_id,
        ...
        VALUES (
            ...
            p_invoice_data->>'supplier_address_raw',
            
            p_invoice_data->>'buyer_name',
            p_invoice_data->>'buyer_company_name',
            p_invoice_data->>'buyer_address',
            p_invoice_data->>'buyer_email',
            p_invoice_data->>'buyer_tax_code',
            (p_invoice_data->>'customer_id')::bigint,
            (p_invoice_data->>'customer_b2c_id')::bigint,
    ...
    ELSE
        ...
        UPDATE public.finance_invoices SET
            ...
            supplier_address_raw = p_invoice_data->>'supplier_address_raw',
            
            buyer_name = p_invoice_data->>'buyer_name',
            buyer_company_name = p_invoice_data->>'buyer_company_name',
            buyer_address = p_invoice_data->>'buyer_address',
            buyer_email = p_invoice_data->>'buyer_email',
            buyer_tax_code = p_invoice_data->>'buyer_tax_code',
            customer_id = (p_invoice_data->>'customer_id')::bigint,
            customer_b2c_id = (p_invoice_data->>'customer_b2c_id')::bigint,
            payment_method = p_invoice_data->>'payment_method',
        ...
        LOOP
            ...
            VALUES (
                ...
                COALESCE(
                  ...
                  (v_item->>'product_unit_id')::bigint
                ),
                COALESCE(v_item->>'name', v_item->>'product_name_raw', v_item->>'vendor_product_name'), 
                COALESCE(v_item->>'xml_unit', v_item->>'unit', v_item->>'internal_unit', v_item->>'vendor_unit'),
                COALESCE(v_item->>'supplier_sku', v_item->>'sku'),
                COALESCE((v_item->>'quantity')::numeric, 0),
                (v_item->>'quantity_buyer')::numeric,
                COALESCE((v_item->>'unit_price')::numeric, (v_item->>'pre_vat_price')::numeric, 0), 
                COALESCE((v_item->>'discount_rate')::numeric, (v_item->>'discount_percentage')::numeric, 0), 
                COALESCE((v_item->>'discount_amount')::numeric, 0),
                COALESCE((v_item->>'vat_rate')::numeric, 0),
                COALESCE((v_item->>'tax_amount')::numeric, (v_item->>'vat_amount')::numeric, 0), 
                COALESCE((v_item->>'amount_before_tax')::numeric, (v_item->>'total_amount_pre_vat')::numeric, 0) 
            );
        END LOOP;
    END IF;
