-- Multi-value catalog filters + country + dosage form (server-side in get_wholesale_catalog)
...
-- Mirror lib/dosage-form.ts detectDosageForm (first match wins)
...
DROP FUNCTION IF EXISTS public.get_wholesale_catalog(
  text, text, text, numeric, numeric, integer, integer, text
);
