-- Migration: extend get_wholesale_catalog sort options
-- Mục đích: thêm sort 'name-desc', 'stock-asc', 'stock-desc', 'expiry-asc', 'expiry-desc'
--           phục vụ FE portal sort server-side thay vì FE-only
-- Strategy: copy full body từ 20260417130000_fix_wholesale_catalog_b2b_stock.sql,
--           chỉ thay ORDER BY block + constant whitelist sort
-- Date: 2026-04-17
...
BEGIN
  ...
  FROM (
    ...
    ORDER BY
      ...
      CASE WHEN p_sort = 'name-desc' THEN p.name END DESC NULLS LAST,
      -- Stock (qty B2B warehouse): asc = ít trước, desc = nhiều trước
      ...
      CASE WHEN p_sort = 'stock-desc' THEN
        COALESCE((SELECT SUM(ib.quantity) FROM public.inventory_batches ib WHERE ib.product_id = p.id AND ib.warehouse_id = v_b2b_wh_id), 0)
      END DESC NULLS LAST,
      -- Expiry (nearest_expiry B2B): asc = gần hết hạn trước
      ...
      CASE WHEN p_sort = 'expiry-desc' THEN
        (SELECT MIN(b.expiry_date) FROM public.batches b
         ...
         WHERE ib.product_id = p.id AND ib.quantity > 0 AND ib.warehouse_id = v_b2b_wh_id)
      END DESC NULLS LAST,
      -- Default (best-seller): out_of_stock xuống dưới, có giá trước
      CASE WHEN p_sort NOT IN ('price-asc','price-desc','newest','name-asc','name-desc','stock-asc','stock-desc','expiry-asc','expiry-desc') THEN
        CASE
