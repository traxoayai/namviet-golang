-- Fix Inbound Detail: trả `unit` từ uom_ordered (ưu tiên) thay vì cột cũ poi.unit
-- Trước đây RPC đọc poi.unit (có DEFAULT 'Hộp') → frontend trang Nhập Kho luôn
-- thấy "Hộp" dù user đã chọn "Lọ"/"Tub" khi tạo PO → process_inbound_receipt
-- validate fail vì product_units không có "Hộp".
-- Date: 2026-04-23
...
BEGIN
    ...
    SELECT jsonb_agg(
        jsonb_build_object(
            ...
            -- [FIX] Ưu tiên uom_ordered (user chọn khi tạo PO) → fallback cột cũ poi.unit
            'unit', COALESCE(NULLIF(poi.uom_ordered, ''), poi.unit),
            -- [NEW] Trả thêm available_units để frontend render dropdown đầy đủ,
            -- user có thể đổi unit ở trang nhập kho nếu cần (đồng bộ với POProductTable).
