-- HOTFIX: process_incoming_bank_transfer multi-order không clamp alloc theo outstanding
-- ============================================================================
-- BUG: Trong multi-order branch, công thức proportional:
--        v_alloc_amount := LEAST(v_remaining, round(p_amount * v_outstanding / v_total_outstanding));
--      Không clamp bởi v_outstanding. Khi p_amount > total_outstanding:
--        - order1 out=20k, total_out=50k, p_amount=80k → alloc = min(80k, 32k) = 32k
--          → ghi tx 32k cho order nợ 20k → overpay 12k, trigger auto_allocate chỉ
--            alloc 20k vào paid_amount → sổ lệch 12k.
--        - order2 out=30k → alloc = min(48k, 48k) = 48k → overpay 18k.
--        - Tổng alloc = 80k → v_remaining=0, không ghi remainder → test fail
--          "excess > 0" + số tiền KH CK dư mất khỏi sổ.
--
-- FIX: LEAST(v_remaining, v_outstanding, proportional) — clamp 3 chiều.
--      Sau loop, v_remaining = p_amount - sum(alloc) có thể > 0 nếu KH CK dư,
--      ghi tx pending remainder để kế toán tra soát.
-- Date: 2026-04-23
-- ============================================================================
