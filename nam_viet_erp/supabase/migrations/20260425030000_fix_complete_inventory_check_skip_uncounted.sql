-- Fix complete_inventory_check v5: SKIP dòng chưa đếm (counted_at IS NULL)
-- ============================================================================
-- BUG nghiêm trọng: Nút "Đủ/OK" và "Bỏ qua" FE chỉ moveToNextItem, KHÔNG commit
-- actual_quantity xuống DB. Items insert từ add_item_to_check_session có default
-- actual_quantity=0, counted_at=NULL. Khi finalize:
--   diff = 0 (actual) - system_quantity → xuất TRẮNG toàn bộ tồn oan.
--
-- Ví dụ đã xảy ra: KK-20260424-032754 / SP Salbutamol 4mg TW2 (10 vỉ x 10 viên):
-- tồn 2900 viên (29 hộp), UI "Khớp số liệu", user bấm "Đủ/OK" → xuất -2900.
--
-- Fix: Chỉ xử lý dòng có counted_at IS NOT NULL. Dòng chưa đếm = giữ nguyên kho.
-- Fix đi kèm FE: "Đủ/OK" phải commit actual=system trước khi next (file tsx).
--
-- Merge-not-replace: giữ nguyên FIX v4 (refresh system_quantity, FEFO, force-sync).
-- Chỉ thêm 1 điều kiện `AND counted_at IS NOT NULL` vào vòng lặp B.
-- Date: 2026-04-25
-- ============================================================================
...
BEGIN
    ...
    -- Đếm số dòng bị skip để report
    SELECT COUNT(*) INTO v_skipped_count
    FROM public.inventory_check_items
    WHERE check_id = p_check_id
      AND counted_at IS NULL;

    -- B. Duyệt qua từng dòng kiểm kê ĐÃ ĐẾM
    -- [FIX v5] SKIP counted_at IS NULL: user chưa đếm/bấm Đủ-OK/Bỏ qua → giữ kho
    FOR v_item IN
