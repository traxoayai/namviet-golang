-- Migration: Fix case-sensitivity bug trong confirm_outbound_packing idempotent check
-- ============================================================================
-- BUG: Migration 20260417115000 check `action_group = 'SALE'` (uppercase only)
--      Nhưng _deduct_stock_fefo (gọi từ create_sales_order và trigger CONFIRMED)
--      ghi txn với `action_group = 'sale'` (lowercase).
--      → Idempotent guard miss → vẫn FEFO trừ lần 2 → báo "thiếu hàng".
-- FIX: Đổi `= 'SALE'` thành `IN ('sale', 'SALE')` ở 2 chỗ (check guard + aggregate).
-- Bonus: thêm NOT LIKE '[REVERTED-DOUBLE-DEDUCT%' để skip txn đã bị revert.
-- Date: 2026-04-17
-- ============================================================================
...
COMMENT ON FUNCTION public.confirm_outbound_packing(UUID) IS
  'V3 (2026-04-17): Idempotent check cover cả action_group ''sale'' (từ _deduct_stock_fefo) và ''SALE'' (từ function này). Loại trừ txn REVERTED.';
