-- Migration: Advisory lock cho confirm_outbound_packing — ngăn double-deduct race
-- ============================================================================
-- BUG GỐC (đã gây 21 đơn double-deduct từ 15/3 đến nay):
--   Function cũ (V3) có idempotent check `v_already_deducted := EXISTS(...)`
--   TRƯỚC khi vào `FOR ... FOR UPDATE` lock `inventory_batches`.
--   Race window:
--     T1 check → v_already_deducted = false
--     T2 check → v_already_deducted = false (đồng thời, chưa commit T1)
--     T1 lock batches → trừ kho → ghi txn → commit
--     T2 lock batches (chờ T1) → trừ kho LẦN 2 → ghi txn thứ 2
--   → 2 sale txn cho cùng order → double-deduct.
--
-- FIX:
--   Dùng `pg_advisory_xact_lock(bigint)` NGAY ĐẦU BODY function.
--   Key derived từ MD5(p_order_id::text) → BIGINT (16 hex đầu ép qua bit(64)).
--   Lock này giữ tới hết transaction → serialize TOÀN BỘ flow packing cho
--   cùng 1 đơn. Cả idempotent check lẫn FEFO deduct nằm trong cùng critical
--   section → T2 phải chờ T1 commit xong mới vào → đọc được txn T1 đã ghi
--   → v_already_deducted = true → rơi vào BRANCH 1 → không trừ lần 2.
--
--   Pattern tham khảo: Odoo stock.move.line._action_done() (SELECT FOR UPDATE
--   ngay đầu), Django ERP (@transaction.atomic + select_for_update), SAP B1
--   (lock row OILM). Postgres-native advisory lock phù hợp hơn SELECT FOR
--   UPDATE khi không có sẵn 1 row để lock (lock theo "intent").
--
-- Body function giữ NGUYÊN 100% V3, chỉ thêm 2 dòng lock ngay sau BEGIN.
-- Date: 2026-04-23
-- ============================================================================
...
COMMENT ON FUNCTION public.confirm_outbound_packing(UUID) IS
  'V4 (2026-04-23): Advisory lock trước idempotent check. Ngăn double-deduct khi 2 session race.';
