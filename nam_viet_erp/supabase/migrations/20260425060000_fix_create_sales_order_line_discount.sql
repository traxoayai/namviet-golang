-- Fix create_sales_order: trừ line-level discount khỏi total_amount
-- ============================================================================
-- BUG CRITICAL: v_total_amount += qty * unit_price → bỏ qua v_item->>'discount'.
-- order_items.discount vẫn lưu đúng, nhưng total_amount và final_amount dội lên
-- cao hơn thực tế → công nợ khách sai lệch (khách bị nợ nhiều hơn).
--
-- Fix: v_total_amount += (qty * unit_price) - COALESCE(line_discount, 0).
-- Merge-not-replace: giữ nguyên TOÀN BỘ logic strict conversion (từ 020000),
-- auth (check_rpc_access), voucher, credit exposure, FEFO deduct, cash tx.
-- Chỉ đổi 1 dòng tại vòng sum total.
--
-- Date: 2026-04-25
-- ============================================================================
