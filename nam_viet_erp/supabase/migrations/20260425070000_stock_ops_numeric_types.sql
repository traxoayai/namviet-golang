-- Migration: Đổi INTEGER → NUMERIC cho v_qty_needed, v_deduct_amount, v_conversion_factor
-- ============================================================================
-- MỤC ĐÍCH:
--   Ba biến khai báo INTEGER trong confirm_outbound_packing bị truncate phần thập phân
--   khi conversion_factor là số thực (vd: 0.5, 1.5) hoặc quantity là NUMERIC.
--   Đổi sang NUMERIC để tính đúng base_qty.
--
-- SCOPE: chỉ thay 3 dòng DECLARE; toàn bộ logic V4 giữ nguyên.
-- Tương đương: V4 body (20260423200100) + đổi type 3 biến.
-- Date: 2026-04-25
-- ============================================================================
...
COMMENT ON FUNCTION public.confirm_outbound_packing(UUID) IS
  'V5 (2026-04-25): v_qty_needed/v_deduct_amount/v_conversion_factor đổi INTEGER → NUMERIC. Tránh truncate phần thập phân khi conversion_factor là số thực.';
