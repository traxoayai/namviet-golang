-- Migration: Drop duplicate get_wholesale_catalog overload (8-param)
-- Merge fuzzy search vào bản 12-param duy nhất
-- Date: 2026-04-14

BEGIN;

-- 1. Drop bản 8-param cũ (thiếu multi-filter params)
DROP FUNCTION IF EXISTS public.get_wholesale_catalog(
  TEXT, TEXT, TEXT, NUMERIC, NUMERIC, INT, INT, TEXT
);

-- 2. Replace bản 12-param: merge fuzzy multi-word search vào
...
BEGIN
  ...
  -- Tách search thành mảng từ (fuzzy multi-word: "eff 150" → match "Efferalgan 150mg")
  IF p_search <> '' THEN
  ...
  WHERE p.status = 'active'
    -- Fuzzy multi-word search: mỗi từ phải match ít nhất 1 field (AND logic)
    AND (p_search = '' OR (
      SELECT bool_and(
        p.name ILIKE '%' || w || '%'
        OR p.sku ILIKE '%' || w || '%'
